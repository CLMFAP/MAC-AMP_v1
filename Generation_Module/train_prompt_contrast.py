...
def load_and_cache_regression_examples(args, filepath, tokenizer):
    data_pos = []
    data_neg = []
    data = []
    data_taski = {}
    ...
    # all_labels = torch.tensor([int(example[2]) for example in data], dtype=torch.long)
    # all_labels = torch.tensor([float(example[2]) for example in data], dtype=torch.float)

    labels = [float(example[2]) for example in data]
    mean_label = np.mean(labels)
    std_label = np.std(labels)

    # 保存均值和方差
    args.label_mean = mean_label
    args.label_std = std_label

    # 使用 z-score 标准化
    norm_labels = [(l - mean_label) / std_label for l in labels]
    all_labels = torch.tensor(norm_labels, dtype=torch.float)
...
def calculate_loss_and_accuracy_(outputs, labels, device, tokenizer):
...
def prompt_contrast_train(args, model, train_dataset, tokenizer):
...
def prompt_contrast_regression_train(args, model, train_dataset, tokenizer):
    ...
    # Freeze all model parameters except soft prompt embeddings
    for param in model.transformer.parameters():
        param.requires_grad = False
    for param in model.lm_head.parameters():
        param.requires_grad = False
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_dataloader:
            ...
            batch = tuple(t.to(device) for t in batch)
            batch_0 = batch[0]
            regression_labels = batch[3].to(torch.float32)
            ...
            outputs_a = model(**inputs_pos)
            loss_a, loss_mask = calculate_loss_and_accuracy_(outputs_a, seq_a, device, tokenizer)
            loss_lengths = torch.sum(loss_mask, 1, keepdim=True).clamp(min=1)

            outputs_b = model(**inputs_neg)
            loss_b, _ = calculate_loss_and_accuracy_(outputs_b, seq_b, device, tokenizer)

            gen_loss_a = loss_a / loss_lengths
            gen_loss_b = loss_b / loss_lengths
            gen_loss = torch.sum(gen_loss_a + gen_loss_b) / bsz
            ...
            # Regression prediction: use difference in loss as prediction
            # predicted_value = (loss_b - loss_a)  # shape: [batch_size]
            predicted_value = torch.tanh(loss_b - loss_a)

            loss_fn = MSELoss()
            regression_loss = loss_fn(predicted_value, regression_labels)

            # Combined final loss
            loss = regression_loss * (1 - args.gen_weight) + args.gen_weight * gen_loss

            if loss.item() > 1e4:
                pred_real = predicted_value * args.label_std + args.label_mean
                print("Huge loss detected")
                print("loss_a", loss_a[:3])
                print("loss_b", loss_b[:3])
                print("predicted", predicted_value[:3])
                print("Real prediction sample:", pred_real[:3].tolist())
                print("label", regression_labels[:3])
            ...
            if batch_steps % args.log_step == 0:
                print(f"train epoch {epoch}/{args.epochs}, batch {batch_steps}/{num_training_steps}, loss {loss.item()}")
...
def setup_args():
    ...
    parser.add_argument('--epochs', default=10, type=int, required=False, help='epochs')
    # parser.add_argument('--warmup_steps', default=500, type=int, required=False, help='warm up steps')
    parser.add_argument('--warmup_steps', default=50, type=int, required=False, help='warm up steps')
    parser.add_argument('--lr', default=1e-2, type=float, required=False, help='learn rate')
...
def finetune_amp_generator_with_data(root_path, train_csv, model_output_dir):
    args = setup_args()
    # args.model_path, args.vocab_path = '', './voc/vocab.txt'
    args.train_raw_path = train_csv
    args.save_model_path = model_output_dir
    ...
    # args = setup_args()
    args.model_path, args.vocab_path = '', f'{root_path}/voc/vocab.txt'
    tokenizer = BertTokenizer(vocab_file=args.vocab_path)
    prompt_model_load = torch.load(f"{root_path}/model_ckpt/pytorch_model.bin")
    model= GPT2LMHeadModel.from_pretrained(f'{root_path}/model_ckpt')
    s_wte = SoftEmbedding(model.get_input_embeddings(),
                          n_tokens=10,
    ...
    train_dataloader = load_and_cache_regression_examples(args, args.train_raw_path, tokenizer=tokenizer)
    prompt_contrast_regression_train(args, model, train_dataloader,tokenizer)

if __name__ == '__main__':
    finetune_amp_generator_with_data()